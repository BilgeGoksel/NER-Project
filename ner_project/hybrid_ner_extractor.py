...
hf_token = "your_hugging_face_token"  # kendi HF token'ını yaz
...
def find_entities(text: str):
    ...
    try:
        ...
        for ent in ner_results:
            if ent.get("entity_group") in LABEL_MAP:
                all_entities.append({
                    "start": ent["start"],
                    "end": ent["end"],
...
# --- Sadece 'Şikayet' sütununu işleyen fonksiyon ---
def process_sikayetvar_excel_to_jsonl(
    input_excel_path="sikayetvar.xlsx",
    text_column="Şikayet",
    output_dir="data",
    ...
    if text_column not in df.columns:
        print(f"HATA: Excel dosyasında '{text_column}' adlı bir sütun yok.")
        return
    ...
    with open(output_jsonl_path, 'w', encoding='utf-8') as outfile:
        for i, metin in tqdm(texts.items(), total=len(texts), desc="Şikayetler işleniyor"):
            try:
                entities = find_entities(metin)
                json_line = {"text": metin, "entities": entities}
...
if __name__ == "__main__":
    process_sikayetvar_excel_to_jsonl(
        input_excel_path="sikayetvar.xlsx",
        text_column="Şikayet",
        output_dir="data",
        output_filename="etiketli_veri.jsonl"
